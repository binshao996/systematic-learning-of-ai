...
import { Send, Square, Loader2, Wrench, Brain, MessageSquare, ChevronDown, ChevronRight } from "lucide-react";
...
export function TestChat({ agentId }: { agentId: string }) {
  ...
  const { running, events, done, error, run, stop } = useAgentRun();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    run(agentId, input.trim());
    setInput("");
  };

  return (
    <div className="border rounded-xl bg-white flex flex-col h-[600px]">
